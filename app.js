...
async function fetchCategoriesAndServices() {
    try {
        ...
        if (!catRes.ok || !svcRes.ok) {
            throw new Error(`HTTP ${catRes.status} / ${svcRes.status}`);
        }
        ...
        document.getElementById("servicesGrid").innerHTML = `
            <p style="color: #ef4444;">
                عذراً، تعذر الاتصال بالسيرفر.<br>
                <small style="color: #94a3b8;">API: ${API_BASE_URL}services/ | خطأ: ${error.message}</small>
            </p>`;
    }
}
